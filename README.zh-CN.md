...
- **不申请任何权限。** 本应用未声明也不请求任何 Android 权限。
- **完全离线运行。** 本应用没有任何网络代码——不会连接互联网，不会向任何服务器发送数据，也不需要网络权限。
- **数据仅保存在本地。** 应用存储的所有内容（收藏夹、界面语言、显示过滤开关）都保存在设备上的应用私有存储（SharedPreferences）中，不会上传到任何地方。
- **只读取系统信息。** 界面展示的值（设备型号、Android 版本、电池状态等）均从设备本地的 `android.os` 类中读取，仅在屏幕上显示，不会离开设备。
- **无广告、无追踪、无统计。** 应用不包含任何第三方 SDK、广告或使用统计。
- 卸载应用时，所有本地数据会自动随之删除。
