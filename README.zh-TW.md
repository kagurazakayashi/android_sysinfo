...
- **不要求任何權限。** 本應用未宣告也不要求任何 Android 權限。
- **完全離線執行。** 本應用沒有任何網路程式碼——不會連線網際網路，不會向任何伺服器傳送資料，也不需要網路權限。
- **資料僅儲存在本機。** 應用程式儲存的所有內容（我的最愛、介面語言、顯示篩選開關）都儲存在裝置上的應用程式私有儲存空間（SharedPreferences）中，不會上傳到任何地方。
- **只讀取系統資訊。** 介面顯示的值（裝置型號、Android 版本、電池狀態等）均從裝置本地的 `android.os` 類別中讀取，僅在螢幕上顯示，不會離開裝置。
- **無廣告、無追蹤、無統計。** 應用程式不包含任何第三方 SDK、廣告或使用統計。
- 解除安裝應用程式時，所有本機資料會自動隨之刪除。
